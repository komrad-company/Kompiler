...
fn exact_is_default_when_no_operator() {
    assert!(matches!(
        find(load("process_filter.yaml"), "hostname").condition,
        FilterTypes::Exact
    ));
}
...
fn contains_operator_parsed_correctly() {
    assert!(matches!(
        find(load("process_filter.yaml"), "proc_name").condition,
...
fn startswith_operator_parsed_correctly() {
    assert!(matches!(
        find(load("process_filter.yaml"), "cmd_line").condition,
...
fn endswith_operator_parsed_correctly() {
    assert!(matches!(
        find(load("process_filter.yaml"), "binary").condition,
...
fn gt_operator_parsed_correctly() {
    assert!(matches!(
        find(load("process_filter.yaml"), "pid").condition,
        FilterTypes::Gt
    ));
}
...
fn gte_operator_parsed_correctly() {
    assert!(matches!(
        find(load("process_filter.yaml"), "min_pid").condition,
        FilterTypes::Gte
    ));
}
...
fn lt_operator_parsed_correctly() {
    assert!(matches!(
        find(load("process_filter.yaml"), "nice_val").condition,
        FilterTypes::Lt
    ));
}
...
fn lte_operator_parsed_correctly() {
    assert!(matches!(
        find(load("process_filter.yaml"), "max_nice").condition,
        FilterTypes::Lte
    ));
}
...
fn scalar_value_is_wrapped_in_vec() {
    assert_eq!(
        find(load("process_filter.yaml"), "cmd_line").values.len(),
        1
    );
}
...
fn multiple_values_all_stored() {
    assert_eq!(
        find(load("process_filter.yaml"), "proc_name").values.len(),
        2
    );
}
...
fn field_name_extracted_correctly() {
    assert!(
        load("process_filter.yaml")
            .0
            .iter()
            .any(|ff| ff.field == "proc_name")
    );
}
